...
const Login =()=>{
    ...
    useEffect(()=>{
        return () =>{
            reset('password', 'confrim')
        }
    },[])

    const regisHandler=(event)=>{
        event.preventDefault();
        post(route('post-user.regis'))
    }
                ...
                <div className="md:w-1/3 md:px-6 flex flex-col justify-between">
                    ...
                    <div>
                        <div className="rounded-[.5rem] w-full flex flex-row bg-base-200 p-2 text-center">
                            <Link className="text-sm w-1/2 p-2 rounded-[.5rem]" href={route('user.login')}>Sign In</Link>
                            <Link className="text-sm w-1/2 p-2 rounded-[.5rem] text-gray-500 bg-[#3498db] text-[#fff]" href={route('user.regis')}>Sign Up</Link>
                        </div>
                        <div>
                            <h2 className="text-xl font-extrabold my-5">Daftar Sekarang</h2>
                            <form action="#" onSubmit={regisHandler}>
                                <div className="flex flex-col my-1.5">
                                    <label htmlFor="name" className="form-control w-full max-w xs">
                                        <div className="label">
                                            <div className="label-text">Nama</div>
                                        </div>
                                        <input type="text" placeholder='Input your name' value={data.name} name="name" id="name" className="input input-bordered" onChange={(e)=>setData('name',e.target.value)}/>
                                    </label>
                                ...
                                <div className="flex flex-col my-1.5">
                                    <label htmlFor="email" className="form-control w-full max-w xs">
                                        ...
                                        <input type="email" placeholder='input your email' value={data.email} name="email" id="email" className="input input-bordered" onChange={(e)=>setData('email',e.target.value)}/>
                                    </label>
                                ...
                                <div className="flex flex-col my-1.5">
                                    <label htmlFor="password" className="form-control w-full max-w xs">
                                        ...
                                        <input type="password" placeholder='input your password' value={data.password} name="password" id="password" className="input input-bordered" onChange={(e)=>setData('password',e.target.value)}/>
                                    </label>
                                    <InputError message={errors.password} className="mt-2" />
                                </div>
                                <div className="flex flex-col my-1.5">
                                    <label htmlFor="confirm" className="form-control w-full max-w xs">
                                        <div className="label">
                                            <div className="label-text">Password Confrimation</div>
                                        </div>
                                        <input type="password" placeholder='confrim your password' value={data.confirm} name="confirm" id="confirm" className="input input-bordered" onChange={(e)=>setData('confirm',e.target.value)}/>
                                    </label>
                                    <InputError message={errors.confirm} className="mt-2" />
                                </div>
                                <button type="submit" className="btn bg-blue-400 hover:bg-blue-500 mx-auto w-full mt-5 py-2 text-base-200">Sign Up</button>
                                <p className="text-sm text-slate-400 text-center my-5">Atau daftar dengan</p>
